...
class CarLights {
  ...
  init() {
    ...
    // copy() is typed to accept an InstancedBufferGeometry, but any
    // BufferGeometry works at runtime — the copied buffers are what matter
    // and instanceCount is overwritten on the next line.
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // Decorative backdrop: cap the pixel ratio so the full-viewport bloom +
    // SMAA chain never pays retina's 4x fill rate (same convention as
    // GuardBackground's dpr={1}).
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const smaaAssets: { search?: HTMLImageElement; area?: HTMLImageElement } = {};
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    myApp.loadAssets().then(() => {
      // If the component unmounted while the SMAA images were loading, dispose
      // has already run — never init a disposed scene.
