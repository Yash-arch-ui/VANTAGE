...
export interface CursorGridProps {
   ...
   * grid keep reacting to the cursor even when the canvas is layered behind
   * interactive content (cards, buttons) or sits in a `pointer-events: none`
...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      const [x, y] = toLocal(e);
      if (propsRef.current.trackWindow) {
        const rect = canvas.getBoundingClientRect();
    ...
    // Listen on `window` when tracking globally, so the grid keeps reacting to
    // the cursor even while the canvas is layered behind interactive content.
    const target: Window | HTMLDivElement = propsRef.current.trackWindow ? window : container;
