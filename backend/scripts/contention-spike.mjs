...
import { privateKeyToAccount, createWalletClient } from 'viem/accounts';
...
async function main() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    } catch (e) { console.error(`Swap ${i+1}/5 failed:`, e.shortMessage || e.message); }
  }
  ...
  for (let i = 0; i < 24; i++) {
    ...
    } catch (e) { console.error(`Spike swap ${i+1}/24 failed:`, e.shortMessage || e.message); }
  }
  ...
  const spikeAlert = alertsJson.items.find(a => a.type === 'contention_spike');
  if (spikeAlert) {
